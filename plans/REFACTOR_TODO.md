...
- [ ] 將 `opperations_of_postgresql()` 函式開頭的 `get_global_data()` 呼叫移除（信任 `init()` 已初始化）
- [ ] 將 `pd.set_option(...)` 從 `opperations_of_postgresql()` 移除，確保只在 `main.py` 的 `__main__` 區塊執行一次
...
| `opperations_of_postgresql.py` | `opperations_of_postgresql()` 及所有 `_` 前綴函式 | `cli/db_menu.py` |
| `opperations_of_postgresql.py` | `get_metadata`, `_listing_metadata`, `_match_table_indexes_and_names` | `service/dataset.py` |
...
- [ ] 驗證：`opperations_of_postgresql.py` 與 `db_maintenance.py` 的現有呼叫正常

### 3-B　搬移 `config/settings.py`

- [ ] 將三個 `__main__` 區塊（`main.py`、`auto_update_data_moa.py`、`opperations_of_postgresql.py`）中重複的 `.env` 載入與 `USERNAME/PASSWORD/DBNAME` 讀取，集中至 `config/settings.py`，各 `__main__` 改為 `from config.settings import USERNAME, PASSWORD, DBNAME`
- [ ] 驗證：三個入口點獨立執行均能正常連線
...
- [ ] 將 `opperations_of_postgresql.py` 的 `get_metadata()`、`_listing_metadata()`、`_match_table_indexes_and_names()` 搬移至 `service/dataset.py`
...
- [ ] `db_maintenance.py` 原位留轉接，`opperations_of_postgresql.py` 的 import 改為新路徑
...
- [ ] 將 `opperations_of_postgresql.py` 的 `opperations_of_postgresql()` 及所有 `_` 前綴函式搬移至 `cli/db_menu.py`，`opperations_of_postgresql.py` 原位留轉接
...
### 資料庫操作選單（`opperations_of_postgresql`）
